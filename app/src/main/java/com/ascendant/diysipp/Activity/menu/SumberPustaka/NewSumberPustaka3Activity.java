...
import com.ascendant.diysipp.Adapter.Home.AdapterSumberPustakaJurusan;
...
public class NewSumberPustaka3Activity extends AppCompatActivity {
    Destiny destiny;
    RelativeLayout Back;
    DB_Helper dbHelper;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_new_sumber_pustaka3);
    ...
    private void Logic2(){
        ...
        login.enqueue(new Callback<ResponseModel>() {
                    ...
                    if (response.body().getStatusCode().equals("000")){
                        try {
                            ...
                            if (response.body().getData().get(0).getPhoto().equals("") || response.body().getData().get(0).getPhoto().isEmpty()){
                                ...
                                Glide.with(NewSumberPustaka3Activity.this)
                        ...
                        Toast.makeText(NewSumberPustaka3Activity.this, response.body().getStatusMessage(), Toast.LENGTH_SHORT).show();
                    }
                }catch (Exception e){
            ...
            public void onFailure(Call<ResponseModel> call, Throwable t) {
                Toast.makeText(NewSumberPustaka3Activity.this, "Koneksi Gagal", Toast.LENGTH_SHORT).show();
                Log.i("Login Logic : ",t.toString());
            }
        });
    }
    private void Logic(){
        mManager = new GridLayoutManager(NewSumberPustaka3Activity.this,1);
        recycler.setLayoutManager(mManager);
        ApiRequest api = RetroServer.getClient().create(ApiRequest.class);
        Call<ResponseModel> Temans = api.SumberPustakaJurusan(destiny.AUTH(Token));
            ...
            public void onResponse(Call<ResponseModel> call, Response<ResponseModel> response) {
                try {
                    if (response.body().getStatusCode().equals("000")){
                        ...
                        mAdapter = new AdapterSumberPustakaJurusan(NewSumberPustaka3Activity.this,mItems,ID_LEMBAGA,ID_KELAS);
                        recycler.setAdapter(mAdapter);
                        mAdapter.notifyDataSetChanged();
                    }else{
                        Toast.makeText(NewSumberPustaka3Activity.this, "Terjadi Kesalahan ", Toast.LENGTH_SHORT).show();
                    }
                }catch (Exception e){
                    Toast.makeText(NewSumberPustaka3Activity.this, "Terjadi Kesalahan User akan Terlogout", Toast.LENGTH_SHORT).show();
                    dbHelper.Logout();
                    Intent intent = new Intent(NewSumberPustaka3Activity.this, LoginActivity.class);
